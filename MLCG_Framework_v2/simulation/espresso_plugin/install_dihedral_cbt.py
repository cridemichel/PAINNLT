...
Con mult < 0 il legame Dihedral(bend=K, mult=-n, phase=phi0) diventa
    ...
    V = K sin^3(t1) sin^3(t2) [1 - cos(n phi - phi0)]

con t1 l'angolo p1-p2-p3 e t2 l'angolo p2-p3-p4 (Bulacu et al., J. Chem.
Theory Comput. 9, 3282 (2013), il potenziale adottato in MARTINI).  Il diedro a
...
sin t = 0,009, forza 10^4).  Con il fattore sin^3 sin^3 la forza va a zero in
modo liscio; il gradiente dei fattori angolari si scrive coi coseni, senza
...
HELPER = r"""/** MLCG CBT: fattore angolare sin^3(t) dell'angolo t fra -u e w (angolo al
...
FORCE_RET_NEW = r"""  /* MLCG CBT: V = bend sin^3(t1) sin^3(t2) [1 - cos(mphi)] se mult < 0 */
  if (mult < 0) {
...
def main() -> None:
    ...
    text = path.read_text()
    if SENTINEL in text:
        print(f"[SKIP] CBT gia' presente: {path}")
        return
