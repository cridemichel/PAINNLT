...
def builder_energy(pos, K, n, phi0):
    return K * (1.0 - np.cos(n * builder_phi(*pos) - phi0))


def builder_forces(pos, K, n, phi0, h=1e-6):
    f = np.zeros_like(pos)
    for a in range(4):
        for x in range(3):
            ...
            pp[a, x] += h
            pm[a, x] -= h
            f[a, x] = -(builder_energy(pp, K, n, phi0) - builder_energy(pm, K, n, phi0)) / (2 * h)
    return f
...
def main():
    ...
    for n in (1, 2):
        for phi0 in (-2.5, -0.4, 0.0, 0.9, 2.8):
            K = 12.0
            bond = espressomd.interactions.Dihedral(bend=K, mult=n, phase=phi0)
            ...
            for _ in range(6):
                ...
                e_ref = builder_energy(pos, K, n, phi0)
                f_ref = builder_forces(pos, K, n, phi0)
                ...
                n_tests += 1
    print(f"  {n_tests} configurazioni, n = 1, 2, cinque fasi: errore relativo massimo "
          f"energia {worst_e:.1e}, forza {worst_f:.1e}")
