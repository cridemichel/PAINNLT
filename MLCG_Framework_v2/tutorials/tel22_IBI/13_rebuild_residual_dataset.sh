...
IBI_VALIDATION_REPORT="${IBI_VALIDATION_REPORT:-ibi_validation_best/validation_report.json}"
cd "${SCRIPT_DIR}"

: "${AA_TOPOLOGY:=md.gro}"
: "${AA_TRAJECTORY:=md_whole.trr}"

for path in "${AA_TOPOLOGY}" "${AA_TRAJECTORY}" tel22_topology.json "${IBI_PRIORS}" "${IBI_VALIDATION_REPORT}"; do
    if [ ! -f "${path}" ]; then
...
"${PYTHON_BIN}" "${PROVENANCE_TOOL}" record \
