...
SOURCE_RUN_DIR="${ALA2_CGNET_EXACT_SOURCE_RUN_DIR:-${ALA2_DIR}/diagnostics/smoke/cgnet_harmonic_50ep}"
RUN_DIR="${ALA2_CGNET_EXACT_RUN_DIR:-${ALA2_DIR}/diagnostics/smoke/cgnet_exact_head_5ep}"
AB_RUN_DIR="${ALA2_CGNET_EXACT_AB_RUN_DIR:-${ALA2_DIR}/diagnostics/smoke/cgnet_exact_head_fes_4x50k}"
CONFIG_SOURCE="${ALA2_CGNET_EXACT_CONFIG_SOURCE:-${ALA2_DIR}/diagnostics/configs/ala2_training_config_cgnet_exact_head_5ep.json}"
MODE=training
...
if [[ "${MODE}" != fes && ! -x "${TRAINER}" ]]; then
    ...
    printf '        Rebuild training/build after applying this patch.\n' >&2
    exit 2
fi
if [[ "${MODE}" != fes && ! -f "${CONFIG_SOURCE}" ]]; then
    printf '[ERROR] Missing CGnet-exact config: %s\n' "${CONFIG_SOURCE}" >&2
    exit 2
fi
...
if [[ "${MODE}" != fes ]]; then
    ...
    if [[ -d "${RUN_DIR}" ]] && find "${RUN_DIR}" -mindepth 1 -print -quit | grep -q .; then
        printf '[ERROR] CGnet-exact run directory is not empty: %s\n' "${RUN_DIR}" >&2
        printf '        Select a fresh ALA2_CGNET_EXACT_RUN_DIR; evidence is never overwritten.\n' >&2
        exit 2
    fi
    ...
    printf '[INFO] Architecture: CGnet-ordered 17-feature head only; Xavier weights; no PaiNN branch.\n'
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/validate_ala2_benchmark.py" \
        ...
        --expected-architecture-variant cgnet_ordered_geometry_tanh_v1 \
        --expected-ordered-energy-scale-kj-mol 4.184 \
        --require-all-to-all \
        --require-cgnet-matched-controls \
        --require-ordered-geometry

    printf '[PASS] CGnet-exact ordered-head training completed in %s\n' "${RUN_DIR}"
else
    for filename in \
        ala2_dataset.bin \
        ala2_priors.json \
        ...
        if [[ ! -s "${RUN_DIR}/${filename}" ]]; then
            printf '[ERROR] Missing completed CGnet-exact artifact: %s\n' "${RUN_DIR}/${filename}" >&2
            exit 2
        fi
    done
    RUN_DIR="$(cd "${RUN_DIR}" && pwd)"
