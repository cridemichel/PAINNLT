...
def main():
    ...
    # diedri stimati (role "twist": fra guanine impilate; "backbone": lungo il
    # filamento) dalla topologia; gli altri dalla base
    fitted_roles = ("twist", "backbone")
    twists = [dict(d) for d in topo.get("dihedrals", []) if d.get("role") in fitted_roles]
    for d in twists:
        if not all(isinstance(d.get(k), (int, float)) for k in ("k", "phi0")):
            sys.exit(f"[ERROR] diedro con parametri non numerici: {d}")
    base_dih = [d for d in priors.get("dihedrals", []) if d.get("role") not in fitted_roles]
    ...
    dih_counts = {r: sum(1 for d in twists if d.get("role") == r) for r in fitted_roles}
    ...
    print(f"[INFO] da {new_topo}: {len(contacts)} contatti {roles}"
          f"{', diedri ' + str(dih_counts) if twists else ''}"
          f"{', DH' if dh else ''}")
