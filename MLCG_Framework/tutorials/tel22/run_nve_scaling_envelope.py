...
plt.scatter(log_dts, log_stds, color='blue', s=100, zorder=5, label='Simulation (Envelope Active)')
...
if np.sum(valid) > 1:
    ...
    x_fit = np.linspace(min(log_dts)-0.1, max(log_dts)+0.1, 100)
...
plt.title('Energy Conservation Scaling (Verlet Integrator + Toxvaerd Cutoff, bias=0)')
plt.grid(True, linestyle=':', alpha=0.7)
plt.legend()
plt.tight_layout()
plt.savefig('energy_conservation_envelope_scaling.png', dpi=300)
print("\n[INFO] Plot saved to energy_conservation_envelope_scaling.png")
