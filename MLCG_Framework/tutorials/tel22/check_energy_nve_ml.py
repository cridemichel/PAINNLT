...
dt_values = [0.0001, 0.0002, 0.0005, 0.001]
std_devs = []

total_time_ps = 1.0

print(f"Starting NVE Energy Conservation Verification on tel22 with ML (Total Time: {total_time_ps} ps)...")

for dt in dt_values:
    steps = int(total_time_ps / dt)
    print(f"Running simulation with dt = {dt} ps (steps = {steps})...")
    
    # Run the MD script WITH --model (ML mode) and with --nve
    cmd = [
...
plt.title(f'NVE Energy Conservation Verification (ML Potential)\nFitted scaling exponent: {slope:.2f}', fontsize=14)
plt.grid(True, which="both", ls="-", alpha=0.5)
plt.legend(fontsize=12)

# Save the plot
plot_filename = "energy_conservation_tel22_nve_ml.png"
