...
int main(int argc, char* argv[]) {
    ...
    std::vector<CGFrame> dataset;
    if (!load_dataset(dataset_file, dataset)) return 1;
    ...
    auto model = std::make_shared<PaiNNModel>(
        config["num_species"], config["hidden_dim"], 3,
        config["num_interactions"], cutoff, config.value("use_bias", false)
    );
