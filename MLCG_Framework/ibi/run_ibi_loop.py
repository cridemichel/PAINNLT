...
np.save('{output_traj}', positions)
""")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, required=True, help="All-Atom target dataset (.bin)")
    parser.add_argument("--priors", type=str, required=True, help="cg_priors.json generated by DBI")
    parser.add_argument("--iterations", type=int, default=3, help="IBI iterations (0 = DBI only)")
    parser.add_argument("--outdir", type=str, default="ibi_priors", help="Output directory")
    parser.add_argument("--bonds", type=str, choices=["IBI", "DBI"], default="IBI", help="Method for bonds")
    parser.add_argument("--angles", type=str, choices=["IBI", "DBI"], default="DBI", help="Method for angles")
    parser.add_argument("--dihedrals", type=str, choices=["IBI", "DBI"], default="DBI", help="Method for dihedrals")
    ...
    print(f"[INFO] Modes -> Bonds: {args.bonds}, Angles: {args.angles}, Dihedrals: {args.dihedrals}")
    ...
    if args.bonds == "IBI":
        ...
        for idx, dists in bond_dists.items():
            if len(dists) == 0: continue
            r, V_0, F_0, P_target = calculate_dbi_potential(dists, bins)
            ibi_tables["bonds"][idx] = {"x": r, "V": V_0, "F": F_0, "P": P_target, "bins": bins}
            filename = f"{args.outdir}/bond_ibi_spline_{idx}.dat"
            save_tabulated_potential(filename, r, V_0, F_0)
            priors_data["bonds"][idx]["type"] = "tabulated"
            priors_data["bonds"][idx]["file"] = filename
            priors_data["bonds"][idx]["min"] = 0.01
            priors_data["bonds"][idx]["max"] = 3.0
            
    if args.angles == "IBI":
        print("[WARNING] Angle IBI requested but usually DBI is preferred.")
        ibi_tables["angles"] = {}
        bins = np.linspace(0.0, np.pi, 100)
        for idx, dists in angle_dists.items():
            if len(dists) == 0: continue
            r, V_0, F_0, P_target = calculate_dbi_potential(dists, bins)
            ibi_tables["angles"][idx] = {"x": r, "V": V_0, "F": F_0, "P": P_target, "bins": bins}
            # Add to priors_data...
            
    if args.dihedrals == "IBI":
        print("[WARNING] Dihedral IBI requested but usually DBI is preferred.")
        ibi_tables["dihedrals"] = {}
        bins = np.linspace(-np.pi, np.pi, 100)
        for idx, dists in dihedral_dists.items():
            if len(dists) == 0: continue
            r, V_0, F_0, P_target = calculate_dbi_potential(dists, bins, periodic=True)
